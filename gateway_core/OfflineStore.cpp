...
bool OfflineStore::insert(const std::string& topic, const std::string& payload) {
    if (!db_) return false;

    const char* sql = "INSERT INTO offline_queue (topic, payload) VALUES (?, ?);";
...
bool OfflineStore::flush(MqttClient& mqtt_client) {
    ...
    const char* select_sql = "SELECT id, topic, payload FROM offline_queue ORDER BY id ASC;";
    sqlite3_stmt* stmt = nullptr;
